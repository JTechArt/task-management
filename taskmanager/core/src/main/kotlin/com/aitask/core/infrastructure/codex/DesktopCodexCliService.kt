package com.aitask.core.infrastructure.codex
...
/**
 * Desktop implementation: resolves `codex` on PATH when [cliPath] is blank, validates with
 * [--version], and opens a terminal (macOS Terminal, Windows cmd, or Linux x-terminal).
 */
class DesktopCodexCliService : CodexCliService {
    ...
    override suspend fun resolveExecutable(cliPath: String?): Result<String> = withContext(Dispatchers.IO) {
        ...
        if (trimmed.isNotEmpty()) {
            ...
            return@withContext when {
                !file.exists() -> Result.failure(IllegalStateException("Codex CLI path does not exist: $trimmed"))
                file.isDirectory -> Result.failure(IllegalStateException("Codex CLI path is a directory: $trimmed"))
    ...
    private fun resolveFromSystemPath(): Result<String> {
        return try {
            val command = if (isWindows) {
                listOf("cmd", "/c", "where codex")
            } else {
                listOf("sh", "-c", "command -v codex")
            }
            ...
            if (!finished) {
                ...
                return Result.failure(
                    IllegalStateException("Codex CLI not found on PATH (lookup timed out). Install Codex or set the executable path in Settings.")
                )
            }
            if (process.exitValue() != 0) {
                return Result.failure(
                    IllegalStateException("Codex CLI not found on PATH. Install Codex or set the executable path in Settings.")
                )
            }
            ...
            logger.warn(e) { "Failed to resolve codex on PATH" }
            Result.failure(
                IllegalStateException("Codex CLI not found on PATH. Install Codex or set the executable path in Settings.", e)
            )
        }
    }
    ...
    override suspend fun validateExecutable(resolvedPath: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (!file.exists()) {
                return@withContext Result.failure(IllegalStateException("Codex CLI not found: $resolvedPath"))
            }
            if (!isWindows && !file.canExecute()) {
                return@withContext Result.failure(IllegalStateException("Codex CLI is not executable: $resolvedPath"))
            }
            ...
            if (!finished) {
                ...
                return@withContext Result.failure(IllegalStateException("Codex CLI did not respond to --version in time."))
            }
            ...
            if (!helpFinished) {
                ...
                return@withContext Result.failure(IllegalStateException("Codex CLI validation timed out."))
            }
            if (helpProbe.exitValue() == 0) {
                return@withContext Result.success(Unit)
            }
            Result.failure(IllegalStateException("Codex CLI failed validation (exit ${versionProbe.exitValue()})."))
        } catch (e: Exception) {
            Result.failure(IllegalStateException("Failed to validate Codex CLI: ${e.message}", e))
        }
    }
    ...
    override suspend fun launchInTerminal(
        ...
        try {
            ...
            logger.error(e) { "Failed to launch Codex in terminal" }
            Result.failure(IllegalStateException("Failed to launch Codex: ${e.message}", e))
        }
    }
    ...
    private fun writeUnixLaunchScript(
        ...
        dir.mkdirs()
        val script = File(dir, "launch-codex.sh")
    ...
    private fun writeWindowsLaunchScript(
        ...
        val dir = File(workspace, ".aitask")
        dir.mkdirs()
        val script = File(dir, "launch-codex.bat")
        val sb = StringBuilder()
