...
class DefaultLlmConnectionValidator(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                ...
                for (candidate in candidates) {
                    val result = probe(candidate, apiKey)
                ...
                throw IllegalStateException(
                    "Unable to reach the configured LLM endpoint. Verify the URL, model server, and any required auth token." +
    ...
    private data class ProbeOutcome(
        val success: LlmConnectionTestResult? = null,
    ...
    private fun probe(candidate: URL, apiKey: String?): ProbeOutcome {
        ...
        return try {
            ...
            if (statusCode in 200..299) {
                ProbeOutcome(
                    success = LlmConnectionTestResult(
                ...
                ProbeOutcome(
                    failureMessage = "HTTP $statusCode from ${candidate}"
                )
            }
        } catch (e: Exception) {
    ...
    private fun normalizeEndpoint(endpointUrl: String): URL {
        ...
        require(trimmed.isNotBlank()) { "LLM endpoint URL must not be blank" }
        val uri = URI(trimmed)
        require(uri.scheme == "http" || uri.scheme == "https") { "LLM endpoint URL must use http or https" }
        return uri.toURL()
    }
}
