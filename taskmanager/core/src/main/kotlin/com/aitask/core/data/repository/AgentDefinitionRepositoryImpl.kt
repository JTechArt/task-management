...
import com.aitask.core.domain.model.AgentToolKindCodec
...
class AgentDefinitionRepositoryImpl : AgentDefinitionRepository {
    ...
    override suspend fun findAvailableForProject(projectId: UUID?, taskType: TaskType?): List<AgentDefinition> =
        newSuspendedTransaction {
            ...
            query.map { it.toDomain() }.filter { agent ->
                taskType == null || agent.taskTypeFilter == null || agent.taskTypeFilter == taskType
            }
        }
    ...
    override suspend fun save(request: AgentDefinitionRequest): AgentDefinition = newSuspendedTransaction {
        ...
        if (existing == null) {
            AgentDefinitions.insert {
                ...
                it[associatedTools] = AgentToolKindCodec.encode(request.associatedTools)
                it[taskTypeFilter] = request.taskTypeFilter?.name
    ...
    private fun ResultRow.toDomain(): AgentDefinition = AgentDefinition(
        ...
        associatedTools = AgentToolKindCodec.decode(this[AgentDefinitions.associatedTools]),
        taskTypeFilter = this[AgentDefinitions.taskTypeFilter]?.let { raw ->
            runCatching { TaskType.valueOf(raw) }.getOrNull()
        },
